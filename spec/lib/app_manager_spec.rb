describe AppManager do 
  ...
  describe "#spin_down" do
    ...
    let(:test_headers) { { authorization: "Bearer #{ENV['HEROKU_AUTH_TOKEN']}", 
                           accept: "application/vnd.heroku+json; version=3", 
                           content_type: 'application/json'} }

    it "correctly calls the formation method on the heroku api via RestClient" do
      RestClient.stub(:patch).with("#{test_uri}/formation", test_data, test_headers) { success_response }
  ...
  describe "#set_config_variable" do
    ...
    it "correctly calls the config-vars method on the heroku api via RestClient" do
      RestClient.stub(:patch).with("#{test_uri}/config-vars", {"GIGITY": "goo"}.to_json, test_headers) { success_response }
